...
from utils.utils import prepare_optimizer, load_checkpoints, load_checkpoints_seq, load_checkpoints_md, save_checkpoints, \
...
from utils.evaluation import evaluate_with_deaminase, evaluate_with_kinase, evaluate_with_cath_more, \
                             ...
                             test_rmsf_cor, test_rmsf_cor_mdcath
...
def training_loop_MD(simclr, start_dms, start_loss, start_rmsf, train_loader, val_loader, test_loader, batch_converter, criterion,
                  optimizer_x, optimizer_seq, scheduler_x, scheduler_seq, train_writer, valid_writer,
                  result_path, logging, configs, replicate, masked_lm_data_collator=None, **kwargs):
    torch.cuda.empty_cache()
    accelerator = kwargs['accelerator']
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if accelerator.sync_gradients:
                ...
                if n_steps % configs.valid_settings.do_every == 0 and n_steps != 0:  # I want to see the step evaluations
                    ...
                    #                              n_steps, logging)
                    val_rmsf_cor = test_rmsf_cor_mdcath(simclr.model_seq.alphabet, configs, simclr.model_seq.esm2,
                                                 n_steps, logging)
            ...
            # n_sub_steps += 1
            # repli_num = configs.train_settings.num_steps if replicate == 2 else (configs.train_settings.num_steps // 3) * (replicate + 1)
            repli_num = configs.train_settings.num_steps
            if n_steps > repli_num:
                break
    ...
    return accelerator, optimizer_x, optimizer_seq, scheduler_x, scheduler_seq, best_val_dms_corr, best_val_loss, best_val_rmsf_cor
...
def main(args, dict_configs, config_file_path):
    ...
    if configs.resume.resume:
        ...
        if configs.model.X_module == 'vivit':
            simclr,start_step = load_checkpoints_seq(simclr, configs, 
                            optimizer_seq,optimizer_x,scheduler_seq,scheduler_x,
                            logging,resume_path=configs.resume.resume_path,restart_optimizer=configs.resume.restart_optimizer)
    ...
    if configs.model.X_module == 'MD':
        ...
        from data.data_MD_mdcath import prepare_dataloaders_mdCATH
        dataloaders = prepare_dataloaders_mdCATH(configs)
    ...
    if accelerator.is_main_process:
        if configs.model.X_module == 'structure':
            ...
            train_steps = np.ceil(len(dataloaders[0][0]) / configs.train_settings.gradient_accumulation)
    ...
    if configs.model.X_module == 'structure':
        ...
        best_dms, best_loss, best_rmsf = 0.0, np.inf, 0.0
        for rep in range(25):
            accelerator, optimizer_x, optimizer_seq, scheduler_x, scheduler_seq, best_dms, best_loss, best_rmsf = training_loop_MD(
                simclr, best_dms, best_loss, best_rmsf, dataloaders[rep][0], dataloaders[rep][1], dataloaders[rep][2], batch_converter, criterion,
                optimizer_x, optimizer_seq, scheduler_x, scheduler_seq, train_writer, valid_writer,
                result_path, logging, configs, replicate=0, masked_lm_data_collator=masked_lm_data_collator, accelerator=accelerator)
