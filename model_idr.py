...
PAD_IDX = len(AA_ORDER)  # padding index
VOCAB_SIZE = PAD_IDX + 1
class Encoder(nn.Module):
    def __init__(self, logging, configs):
        super().__init__()
        if not configs.encoder.random:
            ...
            embed_dim = 1280  # e.g. 128 or 256
            self.random_mlp = nn.Sequential(
    ...
    def one_hot_encode(self, seqs):
        """
        seqs: list[str]
        return: [B, L, VOCAB_SIZE], mask [B, L]
        """
        batch_size = len(seqs)
        max_len = max(len(s) for s in seqs)

        one_hot = torch.zeros(batch_size, max_len, VOCAB_SIZE, device=self.device)
        mask = torch.zeros(batch_size, max_len, device=self.device)

        for i, seq in enumerate(seqs):
            for j, aa in enumerate(seq):
                idx = AA_TO_IDX.get(aa, PAD_IDX)
                one_hot[i, j, idx] = 1.0
                mask[i, j] = 1.0
    ...
    def forward(self, wt_seq, mt_seq):
        if self.use_random:
            wt_onehot, wt_mask = self.one_hot_encode(wt_seq)
            mt_onehot, mt_mask = self.one_hot_encode(mt_seq)

            features_wt = self.random_mlp(wt_onehot)  # [B, L, D]
            features_mt = self.random_mlp(mt_onehot)  # [B, L, D]
        else:
...
def prepare_models(configs, logging):
    ...
    """
    # Prepare the encoder.
    encoder = IDREncoder(logging=logging, configs=configs)
