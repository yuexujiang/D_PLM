...
def prepare_replicate(configs, train_repli_path, test_repli_path):
    if configs.model.MD_encoder.ablation == "permutation":
        samples = prepare_samples(train_repli_path)
    elif configs.model.MD_encoder.ablation == "random":
        samples = prepare_random_samples(train_repli_path)
    ...
    if configs.model.MD_encoder.ablation == "permutation":
        ...
        samples_hard = prepare_random_samples(test_repli_path)
...
def prepare_random_samples(datapath):
    samples=[]
    for file_name in os.listdir(datapath):
        file_path = os.path.abspath(os.path.join(datapath, file_name))
        pid, seq, rep = loadh5(file_path)
        # rep = torch.randn_like(rep)
        rep = np.random.randn(*rep.shape).astype(rep.dtype)
        samples.append((pid, seq, rep))
